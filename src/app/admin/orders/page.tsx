...
export default async function Orders({ searchParams }: { searchParams: Promise<{ f?: string; o?: string }> }) {
  const { f = "awaiting_verification", o: openId } = await searchParams;
  ...
  return (<>
    ...
    <div className="tscroll"><table><thead><tr><th>Order</th><th>When</th><th>Customer</th><th>Items</th><th>Total</th><th>UTR</th><th>Status</th></tr></thead><tbody>
      {list.length ? list.map((x) => (<tr key={x.id}><td><a href={`/admin/orders?f=${f}&o=${x.id}`} style={{ textDecoration: "underline" }}>{x.id}</a></td><td className="small">{new Date(x.created_at).toLocaleDateString("en-IN")}</td>
        ...
        <td>{inr(x.total)}</td><td className="small">{x.utr}</td><td><span className={`status ${x.status}`}>{x.status.replace("_", " ")}</span></td></tr>)) : <tr><td colSpan={7} className="muted">No orders here.</td></tr>}
    </tbody></table></div>
    <p className="small muted" style={{ marginTop: 16 }}>Verify each UTR against your UPI app&apos;s transaction history before confirming. Confirming reduces stock for pieces that aren&apos;t made to order.</p>
    {open && (<>
