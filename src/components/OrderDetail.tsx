...
const LABEL: Record<string, string> = { awaiting_verification: "Awaiting payment verification", confirmed: "Payment confirmed — being made", shipped: "Shipped", rejected: "Payment not found" };
type Props = { o: Partial<Order> & Pick<Order, "id" | "status" | "items" | "total" | "utr" | "upi_id" | "created_at">; admin?: boolean };
...
export default function OrderDetail({ o, admin = false }: Props) {
  return (<>
    ...
    <dl className="small" style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "4px 16px" }}>
      <dt className="muted">UTR</dt><dd style={{ margin: 0 }}>{o.utr}</dd><dt className="muted">Paid to</dt><dd style={{ margin: 0 }}>{o.upi_id}</dd>
